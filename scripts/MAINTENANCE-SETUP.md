...
1. Generate a cryptographically random token (at least 32 bytes). Set the same value in three places: `BTG_MAINTENANCE_TOKEN` in this repository's GitHub Actions secrets, and `MAINTENANCE_TOKEN` on both the `bet-this-guy` and `bet-this-guy-scheduler` Cloudflare Workers. Paste the token itself into each secret's value box, not the secret's name. Never commit, log, or place it in a URL.
2. Run the workflow manually (**Actions**, then **Publish and grade official picks**, then **Run workflow**) and verify all three jobs succeed. Confirm scheduled runs follow without visitors. If networking or provider access fails, repair it before relying on the scheduler.
3. Scheduled runs target every five minutes, offset from the hour. GitHub scheduling is best effort and can be delayed. Check account Actions limits before enabling; public schedules can disable after inactivity. See https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#schedule.

The protected POST endpoint accepts only publish, grade, or closing; never arbitrary picks, odds, results, or code. It returns failure if a job throws, allowing the workflow to retry. Concurrent requests for a given job share work within a Worker instance. Weekly publication's database constraints protect across instances. Grading continues to use provisional/final confirmation rules; a completed job does not mean every pending pick is gradeable. Rotate all three secrets together: `BTG_MAINTENANCE_TOKEN` in GitHub Actions and `MAINTENANCE_TOKEN` on both Cloudflare Workers. A scheduler run that fails with HTTP 401 means the copies don't match.
