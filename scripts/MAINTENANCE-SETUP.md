...
Publishing and grading run from `.github/workflows/btg-maintenance.yml` on this repository's default branch. The website itself is still hosted on Sites. To reinstall the scheduler or rotate its token:

1. Generate a cryptographically random token (at least 32 bytes). Set the same value as secret `MAINTENANCE_TOKEN` in Sites, `BTG_MAINTENANCE_TOKEN` in this repository's GitHub Actions secrets and `MAINTENANCE_TOKEN` on the Cloudflare Worker. Never commit, log, or place it in a URL. Deploy the saved Site version to apply its environment revision.
